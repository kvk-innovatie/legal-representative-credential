...
jq -c '.' "$INPUT_JSON" | while IFS= read -r json; do
  ...
  nested=$(echo "$json" | jq -c '."eu.europa.ec.eudi.legal_representative.nl.1"')
  flattened=$(flatten_json "" "$nested" | jq -s 'add')
  # Merge flattened property back into the original structure
  echo "$json" | jq ".\"eu.europa.ec.eudi.legal_representative.nl.1\" = \$flattened" --argjson flattened "$flattened" > "$OUTPUT_JSON"
done
